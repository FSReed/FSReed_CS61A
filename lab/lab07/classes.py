...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for plcard in player.deck.cards:
            plcard.attack += other_card.attack
            plcard.defense += other_card.defense
            position = 0
        while 1:
            tmp_card = opponent.deck.cards[position]
            if (
                tmp_card.attack == other_card.attack
                and tmp_card.defense == other_card.defense
            ):
                del opponent.deck.cards[position]
            else:
                position = position + 1
            if opponent.deck.is_empty() or position == len(opponent.deck.cards):
                break
