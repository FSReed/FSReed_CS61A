...
def distance(city_a, city_b):
    ...
    distance = sqrt(pow((lat_b - lat_a), 2) + pow((lon_b - lon_a), 2))
    return distance
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    fake_city = make_city("NoName", lat, lon)
    result = None
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        new_branch = [tree(element) for element in leaves]
...
def riffle(deck):
    ...
    return [
        deck[
            position // 2
            + len(deck) // 2 * (position % 2)  # This is the most tricky part!
        ]
...
def construct_sent(word, table):
    ...
    while word not in [".", "!", "?"]:
        "*** YOUR CODE HERE ***"
        result = result + word + " "
        position = random.randrange(0, len(table[word]))
        word = table[word][position]
    return result.strip() + word


def shakespeare_tokens(
    path="shakespeare.txt", url="http://www.composingprograms.com/shakespeare.txt"
):
