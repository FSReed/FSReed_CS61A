...
def prune_min(t):
    ...
    if t.branches == []:
        ...
        for branch in t.branches:
            if branch.label < min_value:
                min_value = branch.label
                branch_left = branch
        prune_min(branch_left)
        t.branches = [branch_left]
...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Have no idea how to solve this. Maybe come back to this problem later.
    # This is the solution from PKUFlyingPig:
    # length = len(s)
    # tot = 0
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        assert isinstance(check, Check), "You should withdraw money from a check."
        if self.holder != check.name or check.deposited:
            print("The police have been notified.")
        else:
            check.deposited = True
            return self.deposit(check.cash)
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            result, cost = skel_char + result, cost
