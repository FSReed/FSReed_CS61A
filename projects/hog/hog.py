...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        ...
        label = GCD(player_score, opponent_score)
        if label >= 10:
            return True
        else:
            return False
...
def GCD(x, y):
    ...
    2
    """
    assert x != 0 and y != 0, "Numbers can't be zero"
    if x == y:
        return x
    elif x < y:
        x, y = y, x
...
def play(
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            ...
            num_roll = strategy0(player_score, opponent_score)
        else:
            player_score = score1
            opponent_score = score0
            num_roll = strategy1(player_score, opponent_score)
        player_score += take_turn(num_roll, opponent_score, dice)
        if who == 0:
            score0 = player_score
        else:
            score1 = player_score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def record(score0, score1):
        if who == 0:
            ...
            score = score1
        point_this_turn = score - last_score
        if point_this_turn > running_high:
            print(point_this_turn, "point(s)! The most yet for Player", who)
            new_highest = point_this_turn
        else:
            new_highest = running_high
        return announce_highest(who, score, new_highest)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def inner_function(*arguments):
        ...
        sum = 0
        while count > 0:
            sum += original_function(*arguments)
            count -= 1
...
def max_scoring_num_rolls(dice=six_sided, trials_count=10000):
    ...
    while count <= 10:
        ...
        average_score = average_dice(count, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    tmp_score = free_bacon(opponent_score) + score
