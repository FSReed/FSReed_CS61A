...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        farthest_place = self.place
        for _ in range(self.min_range):
            init_place = init_place.entrance
            if init_place is beehive:
                return None
        count_place = 0
        while count_place < self.max_range:
            farthest_place = farthest_place.entrance
            count_place += 1
            if farthest_place.entrance is beehive:
                break
        while init_place is not farthest_place:
            if init_place.bees:
                break
            init_place = init_place.entrance
        if init_place.bees:
            return rANTdom_else_none(init_place.bees)
        else:
            return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for every_bee in bee_list:
            every_bee.reduce_armor(total_damage)
        # END Problem 5
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.already_has_a_queen:
            self.is_true = True
            QueenAnt.already_has_a_queen = True
        else:
            self.is_true = False
    ...
    def action(self, gamestate):
        ...
        if not self.is_true:
            ...
            init_place = self.place.exit
            while init_place:
                if init_place.ant:
                    if not init_place.ant.buffed_by_queenant:
                        init_place.ant.damage *= 2
                        init_place.ant.buffed_by_queenant = True
                init_place = init_place.exit
